...
public class Rectangle {
	...
	public bool isIntersecting(Rectangle r)
	{
		...
		if ( ( ( right() >= r.left() && right() <= r.right() ) || (left() >= r.left () && left() <= r.right())) && (( top () <= r.top() && top() >= r.bottom() )  || ( bottom () <= r.top() && ( bottom() >= r.bottom() || top () >= r.top() ))))
			return true;
		else return false;
	}
	
	public bool isInRange(Rectangle r)
	{
		if ( ( ( right() +10 >= r.left() && right()-10 <= r.right() ) || (left()+10>= r.left () && left()-10<= r.right())) && (( top ()-10<= r.top() && top()+10>= r.bottom() )  || ( bottom ()-10<= r.top() && ( bottom()+10 >= r.bottom() || top ()+10>= r.top() ))))
			return true;
		else return false;
	}
